...
@connect(({customer, app, payment, product}) => {
    return {
      ...customer,
...
class ModalSearchProducts extends PureComponent {
  ...
  onChooseProducts = () => {
    runAfterInteractionsBasic(() => {
      const requestBody = checkProductsAndReturnRequestBody(this.props.chooseProducts);
      const {chooseSubscriber} = this.props;
      if (requestBody && requestBody.length > 0) {
        this.closeChooseProductModal()
        this.props.dispatch(createAction('product/showChoosenProduct')())
        this.props.dispatch(createAction('product/calculateServiceProductsFee')({
  ...
  onSubmitEditing = () => {
    runAfterInteractionsBasic(() => {
      this.props.dispatch(createAction('product/showProductList')())
      this.props.dispatch(createAction('product/queryCanBeOrderServiceProducts')({
  ...
  onPressToOpenProduct = (product) => {
    runAfterInteractionsWithToast(() => {
      this.props.dispatch(createAction('product/openProductDetail')({productDetail: product}))
    })
  }

  closeChooseProductModal = () => {
    runAfterInteractionsBasic(() => {
      this.props.dispatch(createAction('product/closeChooseProductModal')())
    })
  }
  ...
  addProduct = (product) => {
    if (this.checkChoosenProduct(product)) {
      runAfterInteractionsWithToast(() => {
        this.props.dispatch(createAction('product/addProduct')({chooseProductTmp: product}))
      })
    }
  }
  ...
  removeProduct = (product) => {
    runAfterInteractionsWithToast(() => {
      this.props.dispatch(createAction('product/removeProduct')({chooseProductTmp: product}))
    })
  }

  chooseValidDuration = (product, validDuration) => {
    runAfterInteractionsWithToast(() => {
      this.props.dispatch(createAction('product/chooseOrderCycle')({
  ...
  choosePricePlan = (product, pricePlan) => {
    runAfterInteractionsWithToast(() => {
      this.props.dispatch(createAction('product/choosePricePlan')({
        productId: product.productId,
  ...
  renderProducts = () => {
    ...
    if (showProductList) {
      return <FlatList
        ...
        onEndReached={(info) => {
          if (count > 12) {
            this.props.dispatch(createAction('product/queryCanBeOrderServiceProductsForPage')({
              queryStr: this.state.searchStr,
  ...
  choosePricePlanInComponent = (packageProduct, component, product, pricePlan) => {
    runAfterInteractionsBasic(() => {
      this.props.dispatch(createAction('product/choosePricePlanInComponent')({
  ...
  chooseValidDurationInComponent = (packageProduct, component, product, validDuration) => {
    runAfterInteractionsBasic(() => {
      this.props.dispatch(createAction('product/chooseOrderCycleInComponent')({
        packageProductId: packageProduct.productId,
        componentId: component.componentId,
        productId: product.productId,
        validDuration: validDuration
  ...
  onPressChooseProductInComponent = (packageProduct, component, product) => {
    runAfterInteractionsBasic(() => {
      this.props.dispatch(createAction('product/addOrRemoveProductInComponent')({
        packageProductId: packageProduct.productId,
        componentId: component.componentId,
        productId: product.productId
      }))
